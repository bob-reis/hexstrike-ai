...
def setup_mcp_server(hexstrike_client: HexStrikeClient) -> FastMCP:
    ...
    def js_intelligence(
        target: str,
        enumdns_target_file: str = "",
        dns_server: str = "",
        depth: int = 3,
        fuzzing: bool = False,
        wayback: bool = False,
        report_formats: str = "html,json",
        proxy: str = "",
        user_agent: str = ""
    ) -> Dict[str, Any]:
        """
        Executa o pipeline de inteligência JavaScript (JS-Analysis) para mapear assets, parâmetros e vulnerabilidades.

        Args:
            target: URL ou domínio base para análise
            enumdns_target_file: Arquivo gerado pelo EnumDNS com hosts priorizados
            dns_server: Resolver customizado, se necessário
            depth: Profundidade de crawling para JS-Discovery
            fuzzing: Ativa fuzzing de diretórios JS
            wayback: Inclui arquivos históricos via Wayback Machine
            report_formats: Formatos do relatório final (ex: "html,json")
            proxy: Proxy HTTP/S opcional (ex: http://127.0.0.1:8080)
            user_agent: User-Agent customizado

        Returns:
            Sumário consolidado da análise, incluindo arquivos gerados.
        """
        ...
        if enumdns_target_file:
            payload["enumdns_target_file"] = enumdns_target_file
        if dns_server:
            payload["dns_server"] = dns_server
        if proxy:
            payload["proxy"] = proxy
        if user_agent:
        ...
        if report_formats:
            if isinstance(report_formats, str):
                payload["report_formats"] = [fmt.strip() for fmt in report_formats.split(',') if fmt.strip()]
            else:
                payload["report_formats"] = report_formats

        logger.info(f"{HexStrikeColors.ELECTRIC_PURPLE}🧠 Iniciando JS intelligence: {target}{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/tools/js-intelligence", payload)

        if result.get("success"):
            analysis = result.get("analysis_summary", {})
            logger.info(
                f"{HexStrikeColors.SUCCESS}✅ JS intelligence concluída | JS files: {analysis.get('js_files', 0)} | "
                f"Endpoints: {analysis.get('endpoints', 0)} | Vulns: {analysis.get('total_vulnerabilities', 0)}{HexStrikeColors.RESET}"
            )
        else:
            logger.error(f"{HexStrikeColors.ERROR}❌ JS intelligence falhou para {target}{HexStrikeColors.RESET}")

        return result

    @mcp.tool()
    def enumdns_operation(
        operation: str = "recon",
        domain: str = "",
        domain_list: str = "",
        wordlist: str = "",
        ...
        dns_port: int = 0,
        proxy: str = "",
        brand_combo: bool = False,
        emit_candidates: bool = False,
        span_last3: bool = False,
        warn_limits: Optional[bool] = None,
        ...
        """
        Execute EnumDNS reconnaissance, brute force, resolve, or threat analysis workflows.

        Args:
            operation: EnumDNS subcommand (recon, brute, resolve, threat-analysis)
            domain: Single domain target (e.g., example.com)
            domain_list: Path to list of domains
            wordlist: Path to wordlist for brute-force mode
            mode: Resolve mode (e.g., bloodhound, file)
            writers: JSON array or comma list describing desired writers
            techniques: JSON array or comma list of threat-analysis techniques
            target_tlds: Comma-separated list of TLDs for threat-analysis
            focus_suffix: PSL suffix focus (e.g., gov.br)
            max_variations: Cap for generated variations in threat-analysis
            dns_server: Override DNS server (host or host:port)
            dns_port: Override DNS port
            proxy: SOCKS/HTTP proxy URI
            brand_combo: Enable brand-combo technique
            emit_candidates: Emit generated candidates before probing
            span_last3: Enable span-last3 flag
            warn_limits: Override warn-limits behaviour (True/False)
            use_recovery: Enable HexStrike recovery handling
            use_cache: Enable command caching
            additional_args: Raw argument string appended to the command
            options: JSON object with advanced options merged into payload

        Returns:
            Response dictionary from the HexStrike EnumDNS endpoint
        """
        ...
        if focus_suffix:
            payload["focus_suffix"] = focus_suffix
        if max_variations:
            payload["max_variations"] = max_variations
        if dns_server:
            payload["dns_server"] = dns_server
        if dns_port:
            payload["dns_port"] = dns_port
        if proxy:
            payload["proxy"] = proxy

        payload["use_recovery"] = use_recovery
        payload["use_cache"] = use_cache

        if brand_combo:
            payload["brand_combo"] = True
        if emit_candidates:
            payload["emit_candidates"] = True
        if span_last3:
            payload["span_last3"] = True
        if warn_limits is not None:
            payload["warn_limits"] = warn_limits

        if writers:
            parsed_writers: Any
            if isinstance(writers, str):
                try:
                    parsed_writers = json.loads(writers)
                except json.JSONDecodeError:
                    parsed_writers = []
                    for item in writers.split(','):
                        item = item.strip()
                        if item:
                            parsed_writers.append({"type": item})
            else:
                parsed_writers = writers
            if parsed_writers:
                payload["writers"] = parsed_writers

        if techniques:
            parsed_techniques: Any
            if isinstance(techniques, str):
                try:
                    parsed_techniques = json.loads(techniques)
                except json.JSONDecodeError:
                    parsed_techniques = [item.strip() for item in techniques.split(',') if item.strip()]
            else:
                parsed_techniques = techniques
            if parsed_techniques:
                payload["techniques"] = parsed_techniques

        if target_tlds:
            if isinstance(target_tlds, str):
                tlds = [item.strip() for item in target_tlds.split(',') if item.strip()]
            else:
                tlds = target_tlds
            if tlds:
                payload["target_tlds"] = tlds

        if options:
            try:
                extra_options = json.loads(options)
                if isinstance(extra_options, dict):
                    payload.update(extra_options)
            except json.JSONDecodeError:
                logger.warning("Failed to parse EnumDNS options JSON; ignoring additional options")

        logger.info(
            f"{HexStrikeColors.NEON_BLUE}🌐 Starting EnumDNS {operation} operation{HexStrikeColors.RESET}"
        )

        result = hexstrike_client.safe_post("api/tools/enumdns", payload)

        if result.get("success"):
            writers_seen = len(result.get("writers", [])) if isinstance(result.get("writers"), list) else 0
            logger.info(
                f"{HexStrikeColors.SUCCESS}✅ EnumDNS {operation} completed | writers: {writers_seen}{HexStrikeColors.RESET}"
            )
            analysis = result.get("analysis_summary")
            if analysis:
                ...
                logger.info(
                    f"{HexStrikeColors.HIGHLIGHT_YELLOW}📈 EnumDNS análise: {high_risk} candidatos de alto risco em {total} variações{HexStrikeColors.RESET}"
                )
                ...
                for candidate in top_candidates[:5]:
                    ...
                    logger.info(
                        f"{HexStrikeColors.CYBER_ORANGE}  ↳ {fqdn} | técnica: {technique} | confiança: {confidence} | risco: {risk_level}{HexStrikeColors.RESET}"
                    )
        else:
            logger.error(
                f"{HexStrikeColors.ERROR}❌ EnumDNS {operation} failed | status: {result.get('return_code')}{HexStrikeColors.RESET}"
            )
    ...
    @mcp.tool()
    def detect_edr_freeze_signals(targets: str = "", include_snapshot: bool = False) -> Dict[str, Any]:
        """
        Collect live process telemetry to highlight potential EDR-Freeze indicators.

        Args:
            targets: Comma-separated list of process names to prioritise (case-insensitive)
            include_snapshot: Include full process snapshot for offline analysis

        Returns:
            Process diagnostic summary with helper correlations and artefact hits
        """
        params: Dict[str, Any] = {}
        if targets:
            params["targets"] = targets
        params["include_snapshot"] = "true" if include_snapshot else "false"

        logger.info("🧠 Gathering EDR-freeze heuristics from server")
        result = hexstrike_client.safe_get("api/processes/edr-freeze-signals", params)
        if result.get("success"):
            helpers = len(result.get("helper_processes", []))
            correlations = len(result.get("correlations", []))
            notes = result.get("analysis_notes", [])
            logger.info(f"✅ Freeze diagnostic snapshot ready | helpers={helpers}, correlations={correlations}")
            if notes:
                logger.info(f"📝 Analysis: {notes[0]}")
        else:
            logger.error("❌ Failed to collect EDR-freeze indicators")
        return result

    @mcp.tool()
    def run_edr_freeze_yara_scan(
        memory_file: str,
        yara_file: str,
        plugin: str = "",
        profile: str = "",
        ...
        """
        Execute Volatility + YARA scan workflows against a memory image to detect freeze artefacts.

        Args:
            memory_file: Path to the memory image to scan
            yara_file: Path to the YARA ruleset
            plugin: Optional Volatility plugin override (defaults to VadYaraScan)
            profile: Optional Volatility profile parameter
            additional_args: Additional CLI arguments appended to the command
            include_matches: Include raw match lines in the response
            use_cache: Allow command result caching on the server

        Returns:
            Scan execution metadata, match summary, and analysis notes
        """
        payload: Dict[str, Any] = {
            "memory_file": memory_file,
            "yara_file": yara_file,
        ...
        logger.info(f"🧬 Starting YARA memory scan against {memory_file}")
        result = hexstrike_client.safe_post("api/forensics/edr-freeze-scan", payload)
        if result.get("success"):
            logger.info(f"✅ YARA scan completed with {result.get('match_count', 0)} matches")
            for note in result.get("analysis_notes", [])[:2]:
                logger.info(f"📝 {note}")
        else:
            logger.error("❌ YARA scan failed to execute")
        return result
